...
- Astro em SSG: `npm run build` gera `dist/`; deploy Cloudflare usa `dist/` e nunca ocorre sem pedido explícito.
- 3 idiomas: PT é a fonte principal; as rotas vivem em `src/pages/` e as cópias editoriais localizadas dos cases em `src/case-studies/`.
- `src/styles/tokens.css` é a fonte única dos tokens. `src/styles/home.css`, `case.css` e `not-found.css` concentram os estilos de cada superfície.
- `src/components/` concentra head, topbars, footer e scripts de comportamento; preservar o contrato de analytics documentado.
...
Ler, nesta ordem: `docs/DESIGN-SYSTEM.md` (tokens e componentes já decididos), `docs/COPY.md` (copies canônicos por idioma), `docs/EDITORIAL-GUIDELINES.md` (diretrizes para copy e narrativa), `docs/DECISIONS.md` (porquês, para não relitigar). Não redescobrir por releitura de CSS o que já está documentado ali.
...
- Para uma ideia (fonte, átomo, molécula, organismo) que precisa amadurecer por mais de uma sessão antes de virar código de produção: usar o agente `sandbox-curator`, que trabalha em `~/Documents/01_Projects/02_Experiments/portfolio-sandbox/` (fora deste repo) com um ledger de maturidade (draft → testing → validated → promoted/discarded). Só lê `docs/DESIGN-SYSTEM.md` inteiro no momento da promoção — nunca antes.
...
- Para revisar copy contra `docs/COPY.md` e `docs/EDITORIAL-GUIDELINES.md`: usar o agente `copy-editor`.

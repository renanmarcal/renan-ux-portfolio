...
### Elemento com `border-left` decorativo precisa de offset negativo pra não descolar do texto ao redor
Qualquer componente que usa `border-left` + `padding-left` pra criar uma "barra de destaque" (ex. `.keypoint` em `case.css`) desloca o *texto* interno para a direita em `padding-left + border-width` — se esse valor não é compensado, o texto do componente não alinha mais com a borda esquerda dos parágrafos/títulos ao redor (foi reportado como elementos "tortos" pelo Renan, 2026-07-21). Correção: manter o `padding-left` (dá respiro entre a barra e o texto) mas somar um `margin-left` negativo de exatamente `-(padding-left + border-width)`, jogando a barra para fora da caixa de conteúdo. O texto volta a alinhar com o `.wrap`/parágrafo irmão; só a barra decorativa fica "flutuando" à esquerda da coluna de leitura.

...
- **Próxima tarefa:** T4 — migrar o case de priorização de visitas nos três idiomas, mantendo as URLs `.html`.
- **Arquivos relevantes:** `src/components/HomePage.astro`, `src/pages/{index,en/index,es/index}.astro`, `src/layouts/HomeLayout.astro`, `src/components/{AnalyticsHead.astro,scripts/HomeBehavior.astro}`, `src/data/locales/{pt,en,es}.ts`, `src/styles/{tokens,home}.css`, `fixtures/public-routes.json`, `scripts/verify-build.mjs`.

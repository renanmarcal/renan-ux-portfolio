...
- **Estado da T7 (2026-07-21):** a branch `chore/astro-migration` foi publicada no GitHub em `3597718` para acionar Workers Builds. A consulta ao Cloudflare via Wrangler não pôde ser autenticada nesta sessão porque `CLOUDFLARE_API_TOKEN` não está disponível. Não houve `wrangler deploy`, promoção para `main` ou deploy de produção. Confirmar o build/URL de preview no painel Cloudflare ou fornecer uma credencial com permissão de leitura para continuar a validação técnica.
